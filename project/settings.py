...
# OIDC settings are defined statically from environment variables because
# mozilla_django_oidc does not support dynamic settings and they need to
# require a server restart to take effect.  They are NOT managed by
# constance for this reason.
...
# Always load mozilla_django_oidc so OIDC can be enabled / disabled at
# runtime without a server restart.  The views and backend check
# settings.OIDC_ENABLED on every request.
