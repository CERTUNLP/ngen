...
const ModalConfirm = (props) => { // props: showModal, onHide, ifConfirm, type, component, state, name
  ...
  useEffect(() => {
    const eventOrCase = props.component.toLowerCase().includes('event')
      ? 'events'
      : 'cases'
      ...
      const fetchTaxonomies = async () => {
        try {
          ...
          const eventData = await Promise.all(
            names.map(url => getTaxonomy(url.trim())))
          const taxonomyUrls = eventData.map(event => event.data.taxonomy)
          // Fetch Domain or Cidr data
          const dOrCidr = eventData.map(
            event => event.data.domain ? event.data.domain : event.data.cidr)
          // Fetch taxonomy data
          const taxonomyData = await Promise.all(
            taxonomyUrls.map(url => getTaxonomy(url.trim())))
  ...
  const type = {
    editState:
      {
        header: `${props.component ? `${t(
          'ngen.edit.header.of')} ${props.component}` : t('ngen.edit.header')}`,
        message: `${props.state ? `${t(
          'ngen.edit.message.disable')} ${props.name}?` : `${t(
          'ngen.edit.message.enable')} ${props.name}?`}`,
        variantButtonConfirm: `${props.state
          ? 'outline-danger'
          : 'outline-primary'}`,
        textButtonConfirm: `${props.state ? t('ngen.disable') : t(
          'ngen.enable')}`,
      },
    delete:
      {
        header: `${props.component ? eventOrCase.includes('event') ? t(
          'ngen.delete.header.event') : t('ngen.delete.header.case') : t(
          'ngen.delete')}`,
    ...
    merge:
      {
        header: `${props.component ? eventOrCase.includes('event') ? t(
          'ngen.merge.header.event') : t('ngen.merge.header.case') : 'Merge'}`,
        message: eventOrCase.includes('event') ? `${t(
          'ngen.merge.message.event')} ${warning}` : `${t(
          'ngen.merge.message.case')} ${warning}`,
  ...
  return (
    <React.Fragment>
      <Modal show={props.showModal} onHide={props.onHide}
        ...
        <Modal.Footer>
          <Button variant={type[props.type].variantButtonConfirm}
                  onClick={props.ifConfirm}>
            {type[props.type].textButtonConfirm}
          </Button>

          <Button variant="outline-secondary" onClick={props.onHide}>{t(
            'ngen.cancel')}</Button>
        </Modal.Footer>
      </Modal>
    </React.Fragment>
