...
const getReports = (currentPage, filters, order) => {
  ...
  return apiInstance.get(
    COMPONENT_URL.report + PAGE + currentPage + '&ordering=' + order + '&' +
...
const getAllReports = (currentPage = 1, results = [], limit = 100) => {
  return apiInstance.get(COMPONENT_URL.report,
...
const postReport = (
  problem, derived_problem, verification, recommendations, more_information,
  lang, taxonomy) => {
  ...
  return apiInstance.post(COMPONENT_URL.report, {
    ...
    if (error.response.status === 400) {
      if (error.response.data.non_field_errors &&
        ...
        'The fields lang, taxonomy must make a unique set.') {
...
const putReport = (
  url, problem, derived_problem, verification, recommendations,
  more_information, lang, taxonomy) => {
