...
const getEdges = (currentPage) => {
  return apiInstance.get(COMPONENT_URL.edge + PAGE + currentPage).then(response => {
    return response;
  }).catch(error => {
...
const getAllEdges = (currentPage = 1, results = [], limit = 100) => {
  return apiInstance.get(COMPONENT_URL.edge, { params: { page: currentPage } })//, page_size: limit
    .then((response) => {
