...
const getMinifiedState = () => {//el parametro es para completar la url con el numero de pagina
  ...
  return apiInstance.get(COMPONENT_URL.stateMinifiedList).then(response => {
...
const getStates = (currentPage, filters, order) => {//el parametro es para completar la url con el numero de pagina
  ...
  return apiInstance.get(
    COMPONENT_URL.state + PAGE + currentPage + '&ordering=' + order + '&' +
...
const getAllStates = (currentPage = 1, results = [], limit = 100) => {
  ...
  return apiInstance.get(COMPONENT_URL.state,
    { params: { page: currentPage, page_size: limit } }).then((response) => {
...
const postState = (name, attended, solved, active, description, children) => {
  ...
  return apiInstance.post(COMPONENT_URL.state, {
    ...
    if (error.response.status === 400) {
      ...
      if (error.response.data.attended && error.response.data.attended[0] ===
        'Must be a valid boolean.') {
        statusText = 'Debe ingresar un valor en el campo \'Atendido\'.'
      } else if (error.response.data.solved && error.response.data.solved[0] ===
        'Must be a valid boolean.') {
        statusText = 'Debe ingresar un valor en el campo \'Resuelto\'.'
      } else if (error.response.data.slug &&
        error.response.data.slug[0].includes(
          'Ya existe una entidad State con slug')) {
...
const putState = (
  url, name, attended, solved, active, description, children) => {
