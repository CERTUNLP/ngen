...
const getMinifiedState = () => {
  ...
  return apiInstance
    .get(COMPONENT_URL.stateMinifiedList)
    .then((response) => {
...
const getStates = (currentPage, filters, order) => {
  //el parametro es para completar la url con el numero de pagina
  let messageError = `No se pudo recuperar la informacion de los estados`;
  return apiInstance
...
const getAllStates = (currentPage = 1, results = [], limit = 100) => {
  return apiInstance
    .get(COMPONENT_URL.state, { params: { page: currentPage, page_size: limit } })
    .then((response) => {
