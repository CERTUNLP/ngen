...
const getMinifiedTaxonomy = () => {
  ...
  return apiInstance.get(COMPONENT_URL.taxonomyMinifiedList).then(response => {
...
const getTaxonomies = (currentPage, filters, order) => {
  ...
  return apiInstance.get(
    COMPONENT_URL.taxonomy + PAGE + currentPage + '&ordering=' + order + '&' +
...
const getAllTaxonomies = (currentPage = 1, results = [], limit = 100) => {
  ...
  return apiInstance.get(COMPONENT_URL.taxonomy,
    { params: { page: currentPage } }) //, page_size: limit
    .then((response) => {
...
const postTaxonomy = (
  type, name, description, active, parent, alias_of, needs_review, group) => {
  ...
  return apiInstance.post(COMPONENT_URL.taxonomy, {
    ...
    if (error.response.status === 400) {
      ...
      if (error.response.data.slug && error.response.data.slug[0].includes(
        'Ya existe una entidad Taxonomy con slug')) {
...
const putTaxonomy = (
  url, type, name, description, active, parent, alias_of, needs_review,
  group) => {
...
const putActivationStatus = (url, state, name) => {
  let messageSuccess = !state
    ? `La taxonomia ${name} ha sido desactivada`
    : `La taxonomia ${name} ha sido activada`
