...
const getTaxonomies = (currentPage, filters, order) => {
  let messageError = `No se pudo recuperar la informacion de las taxonomias`;
  return apiInstance
    .get(COMPONENT_URL.taxonomy + PAGE + currentPage + '&ordering=' + order + '&' + filters)
    .then((response) => {
      return response;
    })
    .catch((error) => {
...
const getAllTaxonomies = (currentPage = 1, results = [], limit = 100) => {
  let messageError = `No se pudo recuperar la informacion de las taxonomias`;
  return apiInstance
    .get(COMPONENT_URL.taxonomy, { params: { page: currentPage } }) //, page_size: limit
    .then((response) => {
