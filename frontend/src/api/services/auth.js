...
import { CLEAR_MESSAGE, LOGOUT, REFRESH_TOKEN } from '../../store/actions';
...
const refreshToken = () => {
  return apiInstance.post(COMPONENT_URL.refreshCookieToken, {}).then(response => {
...
const logout = () => {
  return apiInstance.post(COMPONENT_URL.logout).catch(error => {
    return Promise.reject(error);
  });
}
