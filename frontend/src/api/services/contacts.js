...
const getContacts = (currentPage, filters, order) => {
  ...
  return apiInstance.get(COMPONENT_URL.contact + PAGE + currentPage + '&ordering=' + order + '&' + filters).then(response => {
    return response;
  }).catch(error => {
...
const getMinifiedContact = () => {
  ...
  return apiInstance.get(COMPONENT_URL.contactMinifiedList).then(response => {
