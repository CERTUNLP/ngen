...
const getAllCases = (currentPage = 1, results = [], limit = 100) => {
  return apiInstance
    .get(COMPONENT_URL.case, { params: { page: currentPage } }) //, page_size: limit
    .then((response) => {
...
const getOrderingCases = (currentPage = 1, results = [], limit = 100, id = "+id") => {
  return apiInstance
    .get(COMPONENT_URL.case, { params: { page: currentPage, ordering: id } }) //, page_size: limit
    .then((response) => {
      let res = [...results, ...response.data.results];
      if (response.data.next !== null) {
...
const postCase = (formData) => {
  ...
  return apiInstance
    .post(COMPONENT_URL.case, formData)
    .then((response) => {
...
const patchCaseState = (url, stateUrl) => {
  let messageSuccess = i18next.t("ngen.close.case.success");
  let messageError = i18next.t("ngen.close.case.error") + " .";
  return apiInstance
    .patch(url, { state: stateUrl })
    .then((response) => {
      setAlert(messageSuccess, "success", "case");
      return response;
    })
...
export { getCases, getAllCases, getOrderingCases, getCase, postCase, putCase, deleteCase, mergeCase, patchCase, getMinifiedCase, patchCaseState };
