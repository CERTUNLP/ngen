...
const getMinifiedCase = () => {//el parametro es para completar la url con el numero de pagina
  ...
  return apiInstance.get(COMPONENT_URL.caseMinifiedList).then(response => {
...
const getCases = (currentPage, filters, order) => { //+- id, date, attend_date, priority
  ...
  return apiInstance.get(
    COMPONENT_URL.case + PAGE + currentPage + '&ordering=' + order + '&' +
...
const getOrderingCases = (
  currentPage = 1, results = [], limit = 100, id = '+id') => {
  return apiInstance.get(COMPONENT_URL.case,
    { params: { page: currentPage, ordering: id } })//, page_size: limit
    .then((response) => {
      let res = [...results, ...response.data.results]
      if (response.data.next !== null) {
...
const postCase = (formData) => {
  ...
  return apiInstance.post(COMPONENT_URL.case, formData).then(response => {
