...
const getMinifiedFeed = () => { //+- id, name, slug, created, modified
  ...
  return apiInstance.get(COMPONENT_URL.feedMinifiedList).then(response => {
...
const getFeeds = (currentPage, filters, order) => { //+- id, name, slug, created, modified
  ...
  return apiInstance.get(
    COMPONENT_URL.feed + PAGE + currentPage + '&ordering=' + order + '&' +
...
const putActivationStatus = (url, state, name) => {
  let messageSuccess = !state
    ? `La fuente de informacion ${name} ha sido desactivada`
    : `La fuente de informacion ${name} ha sido activada`
