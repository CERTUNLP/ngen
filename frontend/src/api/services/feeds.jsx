...
const getMinifiedFeed = () => {
  ...
  return apiInstance
    .get(COMPONENT_URL.feedMinifiedList)
    .then((response) => {
...
const getFeeds = (currentPage, filters, order) => {
  //+- id, name, slug, created, modified
  let messageError = `No se pudo recuperar la informacion de las fuentes de informacion`;
  return apiInstance
...
const getAllFeeds = (currentPage = 1, results = [], limit = 100) => {
  let messageError = `No se pudo recuperar la informacion de las fuentes de informacion`;
  return apiInstance
    .get(COMPONENT_URL.feed, { params: { page: currentPage, page_size: limit } })
    .then((response) => {
