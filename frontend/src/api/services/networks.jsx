...
const getNetworks = (currentPage, filters, order) => {
  ...
  return apiInstance.get(
    COMPONENT_URL.network + PAGE + currentPage + '&ordering=' + order + '&' +
...
const postNetwork = (
  children, active, type, parent, network_entity, contacts, address_value) => {
  ...
  return apiInstance.post(COMPONENT_URL.network, {
    ...
    if (error.response.status === 400) {
      if (error.response.data.domain[0] ===
        'Already exists a network with this domain') {
...
const putNetwork = (
  url, children, active, type, parent, network_entity, contacts,
  address_value) => {
...
const isActive = (url, active, name) => {
  let messageSuccess = !active
    ? `La red ${name} ha sido desactivada.`
    : `La red ${name} ha sido activada.`
  let messageError = !active
    ? `La red ${name} no ha sido desactivada. `
    : `La red ${name} no ha sido activada. `
