...
const getTemplates = (currentPage, filters, order) => {
  ...
  return apiInstance.get(
    COMPONENT_URL.template + PAGE + currentPage + '&ordering=' + order + '&' +
...
const postTemplate = (
  address_value, active, priority, event_taxonomy, event_feed, case_lifecycle,
  case_tlp, case_state) => {
  ...
  return apiInstance.post(COMPONENT_URL.template, body).then(response => {
    ...
    if (error.response.status === 400) {
      if (error.response.data.__all__[0] ===
        'CIDR, Domain, Taxonomy, Feed tuple must be unique') {
...
const putTemplate = (
  url, address_value, active, priority, event_taxonomy, event_feed,
  case_lifecycle, case_tlp, case_state) => {
...
const getAllTemplate = (currentPage = 1, results = [], limit = 100) => {
  ...
  return apiInstance.get(COMPONENT_URL.template, {
    params: {
      page: currentPage,
      page_size: limit,
    },
...
const isActive = (url, active) => {
  let messageSuccess = !active
    ? `La plantilla ha sido desactivado`
    : `La plantilla ha sido activado`
