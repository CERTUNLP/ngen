...
const getMinifiedPriority = () => {//el parametro es para completar la url con el numero de pagina
  ...
  return apiInstance.get(COMPONENT_URL.priorityMinifiedList).then(response => {
    return response.data
  }).catch(error => {
    setAlert(messageError, 'error', 'priority')
...
const getPriorities = (currentPage, filters, order) => {//el parametro es para completar la url con el numero de pagina
  ...
  return apiInstance.get(
    COMPONENT_URL.priority + PAGE + currentPage + '&ordering=' + order + '&' +
...
const getAllPriorities = (currentPage = 1, results = [], limit = 100) => {
  ...
  return apiInstance.get(COMPONENT_URL.priority, {
    params: {
      page: currentPage,
      page_size: limit,
    },
    ...
    return Promise.reject(error, 'priority')
  })
...
const postPriority = (name, color, severity, attend_time, solve_time) => {
  ...
  return apiInstance.post(COMPONENT_URL.priority, {
    ...
    setAlert(messageSuccess, 'success', 'priority')
    return response
  }).catch(error => {
    let statusText = ''
    ...
    if (error.response.status === 400) {
      ...
      if (error.response.data.slug &&
        error.response.data.slug[0].substring(0, 39) ===
        'Ya existe una entidad Priority con slug') {
        statusText = 'Ingrese un nombre diferente. '
      } else if (error.response.data.severity &&
        error.response.data.severity[0] ===
        'priority with this severity already exists.') {
        statusText = `Ingrese una severidad diferente.`
        console.log('SEVERIDAD')
      } else if (error.response.data.color && error.response.data.color[0] ===
...
const deletePriority = (url) => {
  ...
  return apiInstance.delete(url).then(response => {
    ...
    let statusText = ''
    if (error.response.data.error && error.response.data.error[0].includes(
      'Cannot delete some instances of model \'Priority\' because they are referenced through protected foreign keys')) {
