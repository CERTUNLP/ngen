...
const getEdges = (currentPage) => {
  return apiInstance.get(COMPONENT_URL.edge + PAGE + currentPage).
    then(response => {
      return response
    }).
    catch(error => {
...
const getAllEdges = (currentPage = 1, results = [], limit = 100) => {
  return apiInstance.get(COMPONENT_URL.edge,
...
const postEdge = (discr, parent, child) => {
  ...
  return apiInstance.post(COMPONENT_URL.edge, {
    ...
    let statusText = ''
    if (error.response.data && error.response.data.non_field_errors[0] ===
      'The fields parent, child must make a unique set.') {
