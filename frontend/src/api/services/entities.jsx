...
const getMinifiedEntity = () => {
  ...
  return apiInstance.get(COMPONENT_URL.entityMinifiedList).then(response => {
...
const getEntities = (currentPage, filters, order) => {
  ...
  return apiInstance.get(
    COMPONENT_URL.entity + PAGE + currentPage + '&ordering=' + order + '&' +
...
const isActive = (url, active, name) => {
  let messageSuccess = !active
    ? `La entidad ${name} se ha desactivado.`
    : `La entidad ${name} se ha activado.`
  let messageError = !active
    ? `La entidad ${name} no se ha desactivado. `
    : `La entidad ${name} no se ha activado. `
