...
function ButtonDelete({ taxonomy }) {
  ...
  const removeTaxonomy = (taxonomy) => {
    deleteTaxonomy(taxonomy.url, taxonomy.name).then(() => {
  ...
  return (
    <>
      ...
      <ModalConfirm type='delete' component='Taxonomia' name={taxonomy.name} showModal={show}
                    onHide={() => handleClose()} ifConfirm={() => removeTaxonomy(taxonomy)}/>
    </>
  );
}
