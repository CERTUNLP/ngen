...
const RowTask = (props) => {  //url, key, taskDeleted,  setTaskDeleted, setTaskUpdated, setShowAlert
  ...
  //Read Task
  const showTaskData = useCallback((url) => {
    getTask(url).then((response) => {
  ...
  const removeTask = (url, name) => {
    deleteTask(url, name).then((response) => {
    ...
    task ?
      <React.Fragment>
        <tr key={task.url}>
          ...
          <td>
            <Form.Control readOnly className="text-center" vertical-align='middle'
                          value={task.description === null ? "No tiene descripcion" : task.description}
                          style={textareaStyle} as="textarea" rows={row}/>
          </td>
          <td>
        ...
        <ModalConfirm showModal={modalDelete} type='delete' component='Task' name={task.name}
                      onHide={() => setModalDelete(false)} ifConfirm={() => removeTask(task.url, task.name)}/>
      </React.Fragment>
      :
      <></>
  );
