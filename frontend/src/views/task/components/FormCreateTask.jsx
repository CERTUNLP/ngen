          ...
          <Col sm={12} lg={12}>
            <Form.Group controlId="Form.Task.Name">
              ...
              <Form.Control
                ...
                isInvalid={(validateUnrequiredInput(props.name))
        ...
        <Row>
          <Col sm={12} lg={12}>
            <Form.Group controlId="Form.Task.Priority">
              <Form.Label>{t('ngen.priority_one')} <b
              ...
              <Form.Control
                ...
                {priorityOption.map((priorityItem, index) => {
                  return (
                    <option key={index}
                            value={priorityItem.url}>{priorityItem.name}</option>
                  )
                })}
              </Form.Control>
              {props.priority ? '' : <div className="invalid-feedback">{t(
                'ngen.priority.select')}</div>}
            </Form.Group>
          </Col>
        </Row>
        <Row>
          <Col sm={12} lg={12}>
            <Form.Group controlId="Form.Task.Description">
              <Form.Label>{t('ngen.description')}<b
                style={{ color: 'red' }}>*</b></Form.Label>
              <Form.Control
                as="textarea"
                ...
                isInvalid={(validateUnrequiredInput(props.description))
                  ? !validateAlphanumeric(props.description)
                  : false}
              />
