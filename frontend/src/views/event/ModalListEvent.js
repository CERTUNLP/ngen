...
const ModalListEvent = (props) => {
  ...
  useEffect(() => {
    getEvents(props.currentPage, props.taxonomyFilter + props.tlpFilter + props.feedFilter + props.wordToSearch + caseIsNull + parentIsNull).then((response) => {
  ...
  return (
    ...
    >
      ...
      <Modal.Body>
        <Row>
          <Col sm={12} lg={12}>
            <Search type={t('ngen.event_one')} setWordToSearch={props.setWordToSearch} wordToSearch={props.wordToSearch}
        ...
        <Row>
          <Col sm={4} lg={4}>
            <FilterSelectUrl options={props.tlpList} itemName={t('ngen.tlp')} partOfTheUrl="tlp"
                             itemFilter={props.tlpFilter}
                             itemFilterSetter={props.setTlpFilter} setLoading={setLoading}
                             setCurrentPage={props.setCurrentPage} value={props.selectTlpFilter}
          ...
          <Col sm={4} lg={4}>
            <FilterSelectUrl options={props.taxonomies} itemName={t('ngen.taxonomy_other')} partOfTheUrl="taxonomy"
                             itemFilter={props.taxonomyFilter} itemFilterSetter={props.setTaxonomyFilter}
                             setLoading={setLoading}
                             setCurrentPage={props.setCurrentPage} value={props.selectTaxonomyFilter}
                             setValue={props.setSelectTaxonomyFilter}/>
          </Col>
          <Col sm={4} lg={4}>
            <FilterSelectUrl options={props.feeds} itemName={t('ngen.feed_other')} partOfTheUrl="feed"
                             itemFilter={props.feedFilter} itemFilterSetter={props.setFeedFilter}
                             setLoading={setLoading}
                             setCurrentPage={props.setCurrentPage} value={props.selectFeedFilter}
