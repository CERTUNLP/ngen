...
const TableEvents = ({
  ...
  return (
    <div>
          ...
          <tr>
            ...
            {!disableDate ?
              (disableOrdering ?
                  <th style={letterSize}>{t('ngen.event.date')} </th>
                  :
                  <Ordering field="date" label={t('ngen.event.date')} order={order} setOrder={setOrder}
                ...
                return event ?
                  (
                    <tr key={index}>
                      ...
                      {!disableDateModified ?
                        <td>{event.modified.slice(0, 10) + " " + event.modified.slice(11, 19)}</td> : ""}
                      {!disableDate ?
                        <td>{event.date ? event.date.slice(0, 10) + " " + event.date.slice(11, 19) : ""}</td> : ""}
                      {!disableUuid &&
                        <td>{event.uuid}</td>
                      ...
                      {!disableTlp &&
                        <td>
                          <LetterFormat useBadge={true} stringToDisplay={tlpNames[event.tlp].name}
      ...
      <ModalConfirm type='delete' component='Evento' name={`${t('ngen.event_one')}`} showModal={remove}
