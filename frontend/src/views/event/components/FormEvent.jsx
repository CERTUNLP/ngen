...
const FormEvent = (props) => {
  const [date, setDate] = useState(
    props.body.date ? props.body.date.substring(0, 16) : getCurrentDateTime())
    ...
    const fetchAllEvidences = async () => {
      try {
        ...
        const responses = await Promise.all(
          props.body.evidence.map((url) => getEvidence(url)))
  ...
  useEffect(() => {
    if (Object.keys(props.priorityNames).length !== 0 &&
  ...
  useEffect(() => {
    ...
    let listDefaultArtifact = props.listArtifact.filter(
      elemento => props.body.artifacts.includes(elemento.value)).
      map(elemento => ({
    ...
    if (event.target.value !== '') {
      postStringIdentifier(event.target.value).then((response) => {
        ...
        setShowErrorMessage(response.data.artifact_type === 'OTHER' ||
          response.data.artifact_type === 'EMAIL')
      }).catch((error) => {
        console.log(error)
      }).finally(() => {
  ...
  const handleClickRadio = (
    event, url, name, date, priority, tlp, state, user) => {
  ...
  const createArtifact = () => {
    ...
    postArtifact(typeArtifact, value).then((response) => {
    ...
    <div>
            ...
            <Row>
              <Col sm={12} lg={4}>
                <Form.Group controlId="formGridAddress1">
                  ...
                  <Form.Control
                    ...
                    name="date"/>
                  {new Date(date) > new Date() ? <div
                    className="invalid-feedback">{t('date.invalid')}</div> : ''}
                </Form.Group>
              </Col>
              <Col sm={12} lg={4}>
                <SelectComponent controlId="exampleForm.ControlSelect1"
                                 label={t('ngen.tlp')} options={props.tlp}
                                 value={selectTlp} nameField="tlp"
              ...
              <Col sm={12} lg={4}>
                <SelectComponent controlId="exampleForm.ControlSelect1"
                                 ...
                                 options={props.taxonomy} value={selectTaxonomy}
                                 nameField="taxonomy"
                                 onChange={completeField1}
                                 placeholder={t('ngen.taxonomy.one.select')}
                                 setOption={setSelectTaxonomy} required={true}
                                 disabled={(props.body.children.length > 0 &&
                                   props.body.children.length > 0)
                                   ? true
                                   : false}/>
              </Col>
            </Row>
            <Row>
              <Col sm={12} lg={4}>
                <SelectComponent controlId="exampleForm.ControlSelect1"
                                 ...
                                 options={props.feeds} value={selectFeed}
                                 nameField="feed"
                                 onChange={completeField1}
              ...
              <Col sm={12} lg={4}>
                <SelectComponent controlId="exampleForm.ControlSelect1"
                                 ...
                                 value={selectPriority} nameField="priority"
                                 onChange={completeField1}
      ...
      {props.disableCardArtifacts ?
        ...
        <Card>
          ...
          <Card.Body>
            <Form.Label>{t('cidr.domain.email')}<b
              style={{ color: 'red' }}>*</b></Form.Label>
            <Row>
              <Col sm={12} lg={6}>
                <Form.Group controlId="formGridAddress1">
                  <Form.Control
                    ...
                    disabled={(props.body.children.length > 0 &&
                      props.body.children.length > 0) ? true : false}
                    onChange={(e) => completeFieldStringIdentifier(e)}
                    isInvalid={showErrorMessage}
                    name="address_value"/>
                  {showErrorMessage ? <div className="invalid-feedback"> {t(
                    'error.ipv4.ipv6.domain')}</div> : ''}
                </Form.Group>
              </Col>
            </Row>
          </Card.Body>
          ...
          <Form>
            <Form.Group controlId="formGridAddress1">
              <Row>
                ...
                <Col sm={12} lg={3}>
                  <CrudButton type="create" name={t('ngen.artifact_one')}
      ...
      {props.disableCardCase ?
        ''
        :
        <SmallCaseTable readCase={caseToLink.value}
                        modalCaseDetail={tableCaseDetail} disableLink={true}
                        modalCase={modalCase} modalListCase={modalListCase}
                        deleteCaseFromForm={deleteCaseFromForm}/>
      }

      <ModalCreateCase showModalCase={showModalCase}
                       setShowModalCase={setShowModalCase} caseItem={caseItem}
                       states={states} setCaseToLink={setCaseToLink}
                       setSelectCase={setSelectCase} selectCase={selectCase}
                       completeField1={completeField1} stateNames={allStates}/>

      <ModalListCase stateNames={allStates}
                     showModalListCase={showModalListCase}
                     modalCaseDetail={modalCaseDetail}
                     closeModal={closeModal} selectedCases={selectedCases}
                     priorityNames={props.priorityNames}
                     tlpNames={props.tlpNames} userNames={props.userNames}
                     handleClickRadio={handleClickRadio}
                     linkCaseToEvent={linkCaseToEvent}
                     completeField1={completeField1} caseToLink={caseToLink}
                     setSelectCase={setSelectCase} selectCase={selectCase}
                     setShowModalListCase={setShowModalListCase}
                     priorities={props.priorities} tlp={props.tlp}
                     allStates={states}
      ...
      <ModalReadCase modalShowCase={modalShowCase} tableDetail={tableDetail}
      ...
      {props.disableCardEvidence ?
        ''
        :
        <EvidenceCard evidences={props.evidence}
                      setEvidences={props.setEvidence}
                      setUpdateCase={props.setUpdateEvidence}
                      updateCase={props.updateEvidence}/>
      }

      <CreateArtifactModal show={modalCreate}
                           onHide={() => setModalCreate(false)} value={value}
                           setValue={setValue}
                           typeArtifact={typeArtifact}
                           setTypeArtifact={setTypeArtifact}
                           createArtifact={createArtifact}/>

      {!(new Date(props.body.date) > new Date()) && props.body.tlp !== '' &&
      props.body.taxonomy !== '' && props.body.feed !== ''
      && props.body.priority !== '' && props.body.address_value !== '' &&
      !showErrorMessage ?
        <Button variant="primary" onClick={props.createEvent}>{t(
          'button.save')}</Button>
        :
