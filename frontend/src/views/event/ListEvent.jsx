...
import FilterSelectWithDefault
  from '../../components/Filter/FilterSelectWithDefault'
...
const ListEvent = () => {
  ...
  const [valueParentIsNull, setValueParentIsNull] = useState(
    { value: 'true', label: t('w.not_assigned') })
  //add to cases
  ...
  useEffect(() => {
    getEvents(currentPage,
      starDateFilter + endDateFilter + taxonomyFilter + tlpFilter + feedFilter +
      caseIsNull + parentIsNull + wordToSearch, order).then((response) => {
  ...
  const completeDateStar = (date) => {
    ...
    if ((endDateFilter !== '') &&
      (starDateFilter !== 'created_range_after=' + date + '&')) { // este if esta porque si no hay cambios en el WordToSearch
  ...
  const completeDateEnd = (date) => {
    ...
    if ((endDateFilter !== 'created_range_before=' + date + '&') &&
      (starDateFilter !== '')) { // este if esta porque si no hay cambios en el WordToSearch
      //haciendo que no se vuelva a ejecutar el useEffect y qeu al setearce setloading en true quede en un bucle infinito
      setFilterDate(true)
      setLoading(true)
    }
      ...
      <Card>
        <Card.Header>
          <Row>
            ...
            <Col sm={8} lg={4}>
              <Search type={t('search.taxonomy_feed_affectedresource')}
          ...
          <Collapse in={open}>
            <div id="example-collapse-text">
              ...
              <Row>
                <Col sm={4} lg={4}>
                  <FilterSelectUrl options={tlpList} itemName={t('ngen.tlp')}
                                   partOfTheUrl="tlp"
                ...
                <Col sm={4} lg={4}>
                  <FilterSelectUrl options={taxonomies}
              ...
              <Row>
                <Col sm={4} lg={4}>
                  <FilterSelect options={types} partOfTheUrl="case__isnull"
                                setFilter={setCaseIsNull}
                                currentFilter={caseIsNull}
                                setLoading={setLoading}
                                placeholder={t('ngen.filter_by') + ' ' +
                                  t('ngen.case_one')}/>
                </Col>
                <Col sm={4} lg={4}>
                  <FilterSelectWithDefault options={types}
        ...
        <Card.Body>
          <TableEvents events={events} loading={loading}
                       selectedEvent={selectedEvent}
                       setSelectedEvent={setSelectedEvent} order={order}
                       setOrder={setOrder}
                       setLoading={setLoading} currentPage={currentPage}
                       taxonomyNames={taxonomyNames}
                       feedNames={feedNames} tlpNames={tlpNames}
        ...
        <ModalConfirm type="merge" component={t('ngen.event_other')}
                      name={selectedEvent} showModal={showModal}
                      onHide={() => setShowModal(false)}
                      ifConfirm={() => merge()}/>

        <Modal show={showOptionsToAddCase} size="lg"
        ...
        <ModalCreateCase showModalCase={showModalCase}
                         ...
                         states={states} setSelectCase={setSelectCase}
                         stateNames={states}
                         evidenceColum={false} buttonsModalColum={false}
                         createCaseModal={true}
                         selectedEvent={selectedEvent}
                         setSelectedEvent={setSelectedEvent} refresh={refresh}
                         setRefresh={setRefresh}/>

        <ModalListCase stateNames={stateNames}
                       showModalListCase={showModalListCase}
                       setShowModalListCase={setShowModalListCase}
                       closeModal={closeModal} setSelectCase={setSelectCase}
                       ...
                       tlpNames={tlpNames} userNames={userNames}
                       priorityNames={priorityNames}
                       priorityFilter={priorityFilter}
                       setPriorityFilter={setPriorityFilter}
                       tlpFilter={tlpFilterCase} setStateFilter={setStateFilter}
                       stateFilter={stateFilter}
                       priorities={allPriorities} tlp={tlpList}
                       allStates={states}
        ...
        />
        <ModalReadCase modalShowCase={modalShowCase}
