    ...
    <Form>
      <Row>
        <Col sm={12} lg={4}>
          <Form.Group controlId="formGridAddress1">
            <Form.Label>{t('ngen.user.username')} <b
            ...
            {validateUserName(body.username) ? '' : <div
              className="invalid-feedback"> {t('validate.username')}</div>}
          </Form.Group>
        </Col>
        <Col sm={12} lg={3}>
          <SelectComponent controlId="exampleForm.ControlSelect1"
                           label="Prioridades" options={priorities}
        ...
        <Col sm={12} lg={5}>
          <Form.Group controlId="formGridEmail">
            ...
            <Form.Control
              ...
              isInvalid={(validateUnrequiredInput(body.email))
                ? !validateUserMail(body.email)
                : false}
            />
            {validateUserMail(body.email) ? '' : <div
              className="invalid-feedback"> {t('w.email.validate')}</div>}
          </Form.Group>
        </Col>
      </Row>
      <Row>
        <Col sm={12} lg={6}>
          <Form.Group controlId="formGridAddress1">
            ...
            <Form.Control
              ...
              onChange={(e) => completeField(e)}
              isInvalid={(validateUnrequiredInput(body.first_name))
                ? !validateName(body.first_name)
                : false}
            />
            {validateName(body.first_name) ? '' : <div
              className="invalid-feedback"> {t('ngen.name.invalid')} </div>}
          </Form.Group>
        </Col>
        ...
        <Col sm={12} lg={6}>
          <Form.Group controlId="formGridAddress1">
            ...
            <Form.Control
              ...
              name="last_name"
              onChange={(e) => completeField(e)}
              isInvalid={(validateUnrequiredInput(body.last_name))
                ? !validateName(body.last_name)
                : false}
            />
            {validateName(body.last_name) ? '' : <div
              className="invalid-feedback"> {t('ngen.name.invalid')} </div>}
          </Form.Group>
        </Col>
      </Row>
      <Row>
        <Col sm={12} lg={6}>
          <Form.Group className="mb-3" controlId="formBasicPassword">
            <Form.Label>{t('ngen.password')} {passwordRequired ? <b
              style={{ color: 'red' }}>*</b> : ''}</Form.Label>
            <Form.Control
              type="password"
              placeholder={passwordRequired
                ? t('ngen.password.placeholder')
                : '********'}
              name="password"
        ...
        <Col sm={12} lg={6}>
          <Form.Group className="mb-3" controlId="formBasicPassword">
            <Form.Label>{t('ngen.password.confirm')} {passwordRequired ?
              <b style={{ color: 'red' }}>*</b> : ''}</Form.Label>
            <Form.Control
              type="password"
              placeholder={passwordRequired
                ? t('ngen.password.placeholder')
...

      {
        ...
        && validateSelect(body.priority) ?
          <><Button variant="primary" onClick={createUser}>{t(
            'button.save')}</Button></>
          :
