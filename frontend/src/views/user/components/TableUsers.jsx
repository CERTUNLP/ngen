    ...
    <div>
      ...
      <ul className="list-group my-4">
        <Table responsive hover className="text-center">
          <thead>
          <tr>
            <Ordering field="username" label={t('ngen.user.username')}
                      order={order} setOrder={setOrder}
                      setLoading={setLoading} letterSize={letterSize}/>
            <Ordering field="email" label={t('w.email')} order={order}
              ...
              <tr key={index}>
                ...
                <td>
                  <ActiveButton active={user.is_active}
                                onClick={() => showModalChangeState(user.url,
                                  user.username, user.is_active)}/>
                </td>
                <td>{user.last_login ? user.last_login.slice(0, 10) + ' ' +
                  user.last_login.slice(11, 19) : 'No inicio sesion'}</td>
                <td>
          ...
          <ModalConfirm type="delete" component={t('ngen.user')}
                        name={deleteUsername} showModal={remove}
                        onHide={() => setRemove(false)}
                        ifConfirm={() => handleDelete(deleteUrl)}/>
          <ModalConfirm type="editState" component={t('ngen.user')}
                        name={dataState.username} state={dataState.state}
                    ...
                    <Card.Body>
                      <Table responsive>
                        ...
                        <tr>
                          ...
                          <td>
                            <Button size="sm" variant="light"
                              ...
                              Casos asignados <Badge variant="light"
                        ...
                        <tr>
                          ...
                          <td>
                            <Form.Control plaintext readOnly
                                          defaultValue={user.modified
                                            ? user.modified.slice(0, 10) + ' ' +
                                            user.modified.slice(11, 19)
                                            : ''}/>
                          </td>
                        </tr>
                      </Table>
