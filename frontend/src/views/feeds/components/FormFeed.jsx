      ...
      <Row>
        <Col sm={12} lg={6}>
          <Form.Group>
            ...
            />
            {validateDescription(props.description) ? '' :
              <div className="invalid-feedback">{t('w.validateDesc')} </div>}
          </Form.Group>
        </Col>
      </Row>
      <Row>
      ...
      <Row>
        <Form.Group as={Col}>
          {props.name !== '' && validateName(props.name) ?
            <Button variant="primary" onClick={props.createFeed}>{t(
              'button.save')}</Button>
            :
            <Button variant="primary" disabled>{t('button.save')}</Button>
          }
          <Button variant="info" href="/feeds">{t('button.cancel')}</Button>
        </Form.Group>
