...
const ListEdge = (props) => {
  ...
  const createEdge = () => {
    ...
    postEdge(edge.discr, props.url, edge.child).then((response) => {
  ...
  return (
    <div>
                  ...
                  <Table responsive hover className="text-center">
                    ...
                    {edges ?
                      edges.map((edge, index) => {
                        return (
                          <RowEdge key={edge.url} url={edge.url} edges={edges} urlByStateName={urlByStateName}
                                   states={states}
                                   listChildren={children} id={index + 1}
                                   edgeDeleted={edgeDeleted} setEdgeDeleted={setEdgeDeleted}
                                   edgeUpdated={edgeUpdated} setEdgeUpdated={setEdgeUpdated}
      ...
      <Modal size='lg' show={modalCreate} onHide={() => closeModal()} aria-labelledby="contained-modal-title-vcenter"
             centered>
        <Modal.Body>
          <Row>
            <Col>
