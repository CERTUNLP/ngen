...
const TableTemplete = ({
  ...
  const changeState = () => {
    ...
    isActive(dataTemplate.url, +!dataTemplate.state).then(() => {
      window.location.href = '/templates';
    }).catch((error) => {
      setShowAlert(true)
      console.log(error)
    ...
    <React.Fragment>
      ...
      <ul className="list-group my-4">
        <Table responsive hover className="text-center">
          <thead>
          <tr>
            <Ordering field="cidr,domain" label={t('ngen.affectedResources')} order={order} setOrder={setOrder}
                      setLoading={setLoading} letterSize={letterSize}/>
            <Ordering field="event_taxonomy__name" label={t('ngen.taxonomy_one')} order={order} setOrder={setOrder}
                      setLoading={setLoading} letterSize={letterSize}/>
            <Ordering field="event_feed__name" label={t('ngen.feed.information')} order={order} setOrder={setOrder}
                      setLoading={setLoading} letterSize={letterSize}/>
            <Ordering field="active" label={t('w.active')} order={order} setOrder={setOrder} setLoading={setLoading}
                      letterSize={letterSize}/>
            <Ordering field="priority" label={t('ngen.priority_one')} order={order} setOrder={setOrder}
                      setLoading={setLoading} letterSize={letterSize}/>
            <Ordering field="case_state" label={t('ngen.state_one')} order={order} setOrder={setOrder}
                      setLoading={setLoading} letterSize={letterSize}/>
            <Ordering field="case_tlp" label={t('ngen.tlp')} order={order} setOrder={setOrder} setLoading={setLoading}
                      letterSize={letterSize}/>
            <Ordering field="matching_events_without_case_count" label={t('ngen.template.matching_events_without_case')}
                      order={order} setOrder={setOrder} setLoading={setLoading} letterSize={letterSize}/>
              ...
              <tr key={index}>
                ...
                <td>
                  <ActiveButton active={+template.active}
                                onClick={() => modalChangeState(template.url, template.cidr, template.domain, taxonomyNames[template.event_taxonomy], feedNames[template.event_feed], template.active)}/>
                </td>
                ...
                <td>
                  {template.matching_events_without_case_count > 0 ?
                    <Button className="" variant="outline-primary" onClick={() => create(template.url)}
                      ...
                      >
                        <path
                          d="M12.645 8.235l-6.647-4.662a1 1 0 0 0-1.618.785v9.324a1 1 0 0 0 1.618.785l6.647-4.662a1 1 0 0 0 0-1.57z"/>
                      </svg>
                    </Button> :
                ...
                <td>
                  ...
                  <CrudButton type='delete'
                              onClick={() => modalDelete(template.cidr, template.domain, taxonomyNames[template.event_taxonomy], feedNames[template.event_feed], template.url)}/>
                </td>
              </tr>
            )
          })}
          ...
          <ModalConfirm type='editState' component={t('ngen.state_one')} name={dataTemplate.cidr || dataTemplate.domain}
                        state={dataTemplate.state} showModal={showTemplate} onHide={() => setShowTemplate(false)}
                    ...
                    <Card.Body>
                      <Table responsive>
                        ...
                        <tr>
                          ...
                          <td>
                            <Button
                              ...
                              <i
                                className={template.active ? 'feather icon-check-circle' : 'feather icon-alert-triangle'}/>
                            </Button>
                          </td>
                        </tr>
                        <tr>
                        ...
                        <tr>
                          ...
                          <td>
                            <Form.Control plaintext readOnly
                                          defaultValue={template.modified ? template.modified.slice(0, 10) + " " + template.modified.slice(11, 19) : ""}/>
                          </td>
                        </tr>
