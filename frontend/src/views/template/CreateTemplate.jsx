...
const CreateTemplate = () => {
  ...
  const createTemplate = () => {
    postTemplate(body.address_value, body.active, body.priority,
  ...
  return (
    <React.Fragment>
      <Alert showAlert={showAlert} resetShowAlert={resetShowAlert}
             component="template"/>
      <Row>
        <Navigation actualPosition={t('ngen.template.add')} path="/templates"
                    index={t('ngen.template_other')}/>
      </Row>
      {loading ?
        <Spinner animation="border"/>
        :
        <FormTemplate body={body} setBody={setBody}
                      createTemplate={createTemplate} tlp={TLP} feeds={feeds}
                      taxonomy={taxonomy} priorities={priorities}
                      states={states}/>
      }
    </React.Fragment>
  )
