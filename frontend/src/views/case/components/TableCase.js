    ...
    <React.Fragment>
      <Table responsive hover className="text-center">
        ...
        <tr>
          ...
          {!disableDate &&
            <Ordering field="date" label={t('ngen.case.management_start_date')} order={order} setOrder={setOrder}
                      setLoading={setLoading} letterSize={letterSize}/>
          }
          {!disableDateModified &&
            <Ordering field="modified" label={t('ngen.date.modified')} order={order} setOrder={setOrder}
                      setLoading={setLoading} letterSize={letterSize}/>
          }
          {!disableUuid &&
        ...
        {list.map((caseItem, index) => {
          ...
          return (
            <tr key={index}>
              ...
              {!disableTlp &&
                <td>
                  <LetterFormat useBadge={true} stringToDisplay={tlpNames[caseItem.tlp].name}
      ...
      <ModalConfirm type='delete' component='Caso' name={`${t('ngen.case_one')}${id}`} showModal={modalDelete}
                    onHide={() => setModalDelete(false)} ifConfirm={() => removeCase(url)}/>
    </React.Fragment>
  );
};
