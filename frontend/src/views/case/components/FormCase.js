...
const FormCase = (props) => {  // props: edit, caseitem, allStates 
  ...
  const [date, setDate] = useState(props.caseItem.date !== null ? props.caseItem.date.substring(0, 16) : getCurrentDateTime())
  ...
  useEffect(() => {
    ...
    if (Object.keys(taxonomyNames).length !== 0 && Object.keys(feedNames).length !== 0 && Object.keys(tlpNames).length !== 0
      ...
      async function fetchAndSetEvents(events) {
        try {
          const responses = await Promise.all(events.map(event => getEvent(event).then((response) => {
  ...
  return (
    <React.Fragment>
      ...
      <ModalListEvent showModalListEvent={showModalListEvent} modalEventDetail={modalEventDetail}
                      ...
                      tlpFilter={tlpFilter} setTlpFilter={setTlpFilter} feedFilter={feedFilter}
