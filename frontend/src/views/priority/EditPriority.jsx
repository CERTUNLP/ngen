...
const EditPriority = () => {
  ...
  useEffect(() => {
    ...
    if (body.attend_time !== '') {
      attend_time_days = body.attend_time.split(' ')[1]
        ? body.attend_time.split(' ')[0]
        : ''
      attend_time_hours = body.attend_time.split(' ')[1]
        ? body.attend_time.split(':')[1]
        : body.attend_time.split(':')[0]
      attend_time_minutes = body.attend_time.split(' ')[1]
        ? body.attend_time.split(':')[2]
        : body.attend_time.split(':')[1]
    }
    if (body.solve_time !== '') {
      solve_time_days = body.solve_time.split(' ')[1] ? body.solve_time.split(
        ' ')[0] : ''
      solve_time_hours = body.solve_time.split(' ')[1] ? body.solve_time.split(
        ':')[1] : body.solve_time.split(':')[0]
      solve_time_minutes = body.solve_time.split(' ')[1]
        ? body.solve_time.split(':')[2]
        : body.solve_time.split(':')[1]
    }
    const form = {
      url: body.url,
      name: body.name,
