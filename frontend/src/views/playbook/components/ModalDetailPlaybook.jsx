                  ...
                  <Row>
                    <Col>
                      ...
                      <span className="d-block m-t-5">{t(
                        'ngen.playbook.detail')}</span>
                    </Col>
                    <Col sm={12} lg={2}>
                      <Link to="/playbooks/edit" state={props.playbook}>
                        <CrudButton type="edit"/>
                      </Link>
                      <CloseButton aria-label={t('button.close')}
                  ...
                  <Table responsive>
                      ...
                      <tr>
                        ...
                        <td>
                          <Form.Control plaintext readOnly
                                        defaultValue={props.playbook.name}/>
                        </td>
                      </tr>
                      :
                      <></>
                    }
                    {props.playbook.taxonomy && props.playbook.taxonomy.length >
                    0 ?
                      <tr>
                        <td>{t('ngen.taxonomy_other')}</td>
                        <td>
                          {Object.values(props.playbook.taxonomy).
                            map((taxonomyItem, index) => {
                              return (
                                <FormGetName form={true} get={getTaxonomy}
                                             url={taxonomyItem} key={index}/>
                              )
                            })
                          }
                        </td>
                    ...
                    {props.playbook.tasks && props.playbook.tasks.length > 0 ?
                      <tr>
                        <td>{t('ngen.tasks')}</td>
                        <td>
                          {Object.values(props.playbook.tasks).
                            map((taskItem, index) => {
                              return (
                                <FormGetName form={true} get={getTask}
                                             url={taskItem} key={index}/>
                              )
                            })
                          }
                        </td>
