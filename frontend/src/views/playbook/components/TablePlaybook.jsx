...
const TablePlaybook = ({ setIsModify, list, loading, taxonomyNames }) => {
  ...
  const removePlaybook = (url, name) => {
    deletePlaybook(url, name).then((response) => {
  ...
  return (
    <React.Fragment>
            ...
            <tr key={index}>
              ...
              <td>
                ...
                <CrudButton type="delete"
                            onClick={() => Delete(book.url, book.name)}/>
              </td>
            </tr>
          )
        })}
      ...
      <ModalDetailPlaybook show={modalShow} playbook={playbook}
                           onHide={() => setModalShow(false)}/>
      <ModalConfirm type="delete" component="Playbook" name={name}
