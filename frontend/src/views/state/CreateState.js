...
const AddState = () => {
  ...
  const createState = () => {
    postState(body.name, body.attended, body.solved, 1, body.description, body.children).then((response) => {
  ...
  const editState = () => {
    putState(url, body.name, body.attended, body.solved, 1, body.description, body.children).then((response) => {
      // setState(response.data) // y la url
      setBody(response.data)
      setUrl(response.data.url)
      setSectionAddEdge(true)
  ...
  return (
    <div>
      ...
      <FormState body={body} setBody={setBody} createState={!sectionAddEdge ? createState : editState}
                 type={t('w.add')}/>
