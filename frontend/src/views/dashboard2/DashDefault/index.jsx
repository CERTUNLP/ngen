...
const DashDefault = () => {
  ...
  useEffect(() => {
    ...
    getDashboardFeed(starDateFilter + endDateFilter).then((response) => {
    ...
    getDashboardEvent(starDateFilter + endDateFilter).then((response) => {
    ...
    getDashboardCases(starDateFilter + endDateFilter).then((response) => {
    ...
    getDashboardNetworkEntities(starDateFilter + endDateFilter).
      then((response) => {
        ...
        const entitiesWithEventCount = response.data.network_entities.map(
          entity => {
  ...
  const completeDateEnd = (date) => {
    console.log(endDate)
    if (getCurrentDateTime() >= date && date >= starDate && endDate >=
      starDate) {
      console.log(endDate)
        ...
        <Col sm={12} lg={6}>
          <Form.Group controlId="formGridAddress1">
            ...
            <Form.Control
              ...
              name="date"
            />
            {starDateNotification ? <div className="invalid-feedback">{t(
              'date.invalid')}</div> : ''}
          </Form.Group>
        </Col>
        <Col sm={12} lg={6}>
          <Form.Group controlId="formGridAddress1">
            ...
            <Form.Control
              ...
              name="date"
            />
            {endDateNotification ? <div className="invalid-feedback"> {t(
              'date.invalid')}</div> : ''}
          </Form.Group>
        </Col>
      </Row>
      <Row>
        <Col md={6}>
          <Card>
            <Card.Header>
              <Card.Title as="h5">{t(
                'ngen.dashboard.feeds_graphic')}</Card.Title>
            </Card.Header>
        ...
        <Col md={6}>
          <Card>
            <Card.Header>
              <Card.Title as="h5">{t(
                'ngen.dashboard.entities_graphic')}</Card.Title>
            </Card.Header>
            <Card.Body className="text-center">
              <EntityGraph list={dashboardNetworkEntities.length > 0
                ? dashboardNetworkEntities[0].values
                : []}/>
            </Card.Body>
          </Card>
        </Col>
        <Col>
          <DashboardEvent list={dashboardEvent} feedNames={feedNames}
