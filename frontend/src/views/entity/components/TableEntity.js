...
const TableEntity = ({ setIsModify, list, loading, setLoading, currentPage, order, setOrder }) => {
  ...
  return (
    <React.Fragment>
                      ...
                      <td>
                        <Button size="sm" variant='light' className="text-capitalize">
                          {t('ngen.network_other')} <Badge variant="light"
                                                           className="ml-1">{entity ? entity.networks.length : 0}</Badge>
                        </Button>
                      </td>
                    </tr>
                    </tbody>
      ...
      <ModalConfirm type='delete' component={t('ngen.entity')} name={name} showModal={modalDelete}
                    onHide={() => setModalDelete(false)} ifConfirm={() => removeEntity(url, name)}/>

      <ModalConfirm type='editState' component={t('ngen.entity')} name={name} state={active} showModal={modalState}
                    onHide={() => setModalState(false)} ifConfirm={() => switchState(url, active, name)}/>
