...
const NavContent = ({ navigation }) => {
  ...
  const navItemsBottom = navigation.itemsBottom.map((item) => {
    switch (item.type) {
      case 'group':
        return <NavGroup layout="vertical" key={'nav-group-' + item.id}
                         group={item}/>
      default:
        return false
    }
  ...
  mainContent = (
    <div className="navbar-content datta-scroll">
      <PerfectScrollbar>
        <ListGroup variant="flush" as="ul" bsPrefix=" "
                   className="nav pcoded-inner-navbar" id="nav-ps-next">
          {navItems}
        </ListGroup>
        <ListGroup variant="flush" as="ul" bsPrefix=" "
                   className="nav pcoded-inner-navbar" id="nav-ps-bottom">
          {navItemsBottom}
        </ListGroup>
        <NavCard/>
      </PerfectScrollbar>
