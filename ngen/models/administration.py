...
class Priority(AuditModelMixin, SlugModelMixin, ValidationModelMixin):
    ...
    attend_time = models.DurationField(
        default=timedelta(minutes=config.PRIORITY_ATTEND_TIME_DEFAULT)
    )
    solve_time = models.DurationField(
        default=timedelta(minutes=config.PRIORITY_SOLVE_TIME_DEFAULT)
    )
