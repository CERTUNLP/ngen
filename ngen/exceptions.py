...
def django_error_handler(exc, context):
    ...
    if response is None and isinstance(exc, core_ValidationError):
        ...
        if 'duplicate key value violates unique constraint' in error_message:
            try:
                klass = context.get('view').queryset.model.__name__
                key = error_message.split('Key (')[1].split(')')[0].strip()
                value = error_message.split(')=(')[1].split(')')[0].strip()
                return Response(status=status.HTTP_400_BAD_REQUEST, data={key: [f'Ya existe una entidad {klass} con {key}={value}.'], '_detail': error_message})
            except:
            ...
            return Response(status=status.HTTP_400_BAD_REQUEST, data={'__all__': [f'No se puede editar y/o eliminar la entidad porque hay objetos que dependen de ella.'], '_detail': error_message})
        else:
