...
class AnnouncementTestCase(TestCase):
    ...
    def test_case_close_email(self):
        ...
        self.assertEqual(
            len(mail.outbox), 2
        )  # Test if inbox has 2 emails, one for open and other for close.
        self.assertIn(
            "Case closed", mail.outbox[1].subject
        )  # Check for the title of the second email.
    ...
    def test_case_initial_staging(self):
        ...
        self.assertEqual(
            len(mail.outbox), 0
        )  #  No email for Initial> Staging. FAIL: 2 emails: New Case + Case Status Updated.
    ...
    def test_case_update_email(self):
        ...
        self.case.state = State.objects.get(
            name="Staging"
        )  # Change to staging to force update from hook.
        self.case.before_update()
        self.assertEqual(
            len(mail.outbox), 3
        )  # New open case > Case Opened > X attended X solved for update
