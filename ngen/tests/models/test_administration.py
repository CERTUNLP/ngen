...
class TestAdministration(TestCase):
    ...
    def test_priority_model(self):
        ...
        self.assertEqual(
            self.priority.attend_time,
            timedelta(minutes=config.PRIORITY_ATTEND_TIME_DEFAULT),
        )
        # AttributeError: 'Settings' object has no attribute 'PRIORITY_ATTEND_TIME_DEFAULT'
        self.assertEqual(
            self.priority.solve_time,
            timedelta(minutes=config.PRIORITY_SOLVE_TIME_DEFAULT),
        )
