...
class StateTestCase(TestCase):
    ...
    def setUpTestData(cls):
        ...
        default_priority, created = Priority.objects.get_or_create(
            name=config.PRIORITY_DEFAULT, severity=1
        )
    ...
    def test_state_deletion(self):
        ...
        edge = Edge.objects.create(
            parent=state_to_delete, child=another_state, discr="Test Edge"
        )

        # Check that the edge exists
        self.assertTrue(
            Edge.objects.filter(parent=state_to_delete, child=another_state).exists()
        )
    ...
    def test_edge_creation(self):
        ...
        """
        edge = Edge.objects.create(
            parent=self.first_state, child=self.second_state, discr="Test Edge"
        )
    ...
    def test_edge_deletion(self):
        ...
        self.assertFalse(
            Edge.objects.filter(
                parent=self.first_state, child=self.second_state
            ).exists()
        )
