...
class UserFilterTest(BaseFilterTest):
    ...
    def setUpTestData(cls):
        ...
        cls.queryset = User.objects.exclude(username="ngen")  # Exclude fixture user
    ...
    def test_search_filter(self):
        ...
        self.assertEqual(
            self.get_id_from_url(response.data["results"][0]["url"]), self.user_1.id
        )
        ...
        self.assertEqual(
            self.get_id_from_url(response.data["results"][1]["url"]), self.user_2.id
        )
        ...
        self.assertEqual(response.data["count"], 1)
        self.assertEqual(
            self.get_id_from_url(response.data["results"][0]["url"]), self.user_2.id
        )
    ...
    def test_filter_by_modified_range(self):
        ...
        self.assertQuerysetEqual(
            filtered_queryset, [self.user_1, self.user_2, self.user_3], ordered=False
        )
    ...
    def test_filter_by_username(self):
        ...
        params = {"username__icontains": "two"}

        filtered_queryset = self.filter(params).qs

        self.assertQuerysetEqual(filtered_queryset, [self.user_2])

        params = {"username__icontains": "o"}
    ...
    def test_filter_by_email(self):
        ...
        params = {"email__icontains": "one"}

        filtered_queryset = self.filter(params).qs

        self.assertQuerysetEqual(filtered_queryset, [self.user_1])

        params = {"email__icontains": "outlook"}

        filtered_queryset = self.filter(params).qs

        self.assertQuerysetEqual(filtered_queryset, [self.user_3], ordered=False)
    ...
    def test_filter_by_first_name(self):
        ...
        params = {"first_name__icontains": "one"}

        filtered_queryset = self.filter(params).qs

        self.assertQuerysetEqual(filtered_queryset, [self.user_1])

        params = {"first_name__icontains": "o"}
    ...
    def test_filter_by_last_name(self):
        ...
        params = {"last_name__icontains": "one"}

        filtered_queryset = self.filter(params).qs

        self.assertQuerysetEqual(filtered_queryset, [self.user_1])

        params = {"last_name__icontains": "o"}
    ...
    def test_filter_by_is_staff(self):
        ...
        self.assertQuerysetEqual(filtered_queryset, [self.user_3])
